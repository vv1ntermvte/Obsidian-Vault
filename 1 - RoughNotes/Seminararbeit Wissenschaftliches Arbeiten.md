...
Preliminaries:
1. Knowledge Graph: KGs are graph structures, that combines entities, the nodes of the graph, over relations, the edges of the graph, with one another, to store large amounts of knowledge. Usually, a KG $G$ is defined through knowledge triples of the form $(h, r, t)$, with $h$ and $t$ being the head and tail entity, respectively, and $r$ being a relation. A given $(h, r, t)$-triple denotes the existence of a relation of the form $r$ between $h$ and $t$ within $G$. A set of such triples hence constructs the complete graph. 
2. Knowledge Graph Question Answering: KGQA is a common task to test the utilization of knowledge a LLM could retrieve from a KG. It consists of asking various questions that are based on knowledge that stems from the KG, with the goal of establishing a function capable of predicting answers to these questions and maximizing the amount of correct answers generated by said function.
3. Anchor Entities: For a given question, the Anchor Entities are the set of entities contained in $G$, that are already part of the question itself. E.g. within the Question 'The Artist who drew "The Starry Night" was born in which Country?', the sole Anchor Entity would be /The Starry Night'.
4. Reasoning Path: For a given KG, a Reasoning Path consists of a sequence of entities, that are connect with one another via relations. This is represented as a ordered list of triples, with the tail entity of one triple being identical with the head entity of the subsequent one. Reasoning paths can serve as possible solutions for KG based questions. In this case, they start of from an anchor entity, connecting it with the possible solution. For the aforementioned question, a reasoning path to the correct answer may look like this: $(The\_ Starry\_Night, drawn\_ by, Van\_ Gogh), (Van\_ Gogh, born\_ in, Netherlands)$
...
While both methodologies prioritize preserving structural dependencies to keep the retreived knowledge meaningful and relevant, they employ fundamentally different mechanisms on their way to achieving results. LightPROF targets efficiency for small-scale, open-source models by training a customized adapter that maps graph structures directly into the LLM’s embedding space, thereby significantly reducing token consumption. Conversely, PoG utilizes a training-free, natural language based approach compatible with closed-source architectures. By leveraging the LLM to plan and prune explicit reasoning paths, PoG ensures interpretability but requires a higher computational load. These differences stress the critical trade-off between the high-efficiency, knowledge embedding approach of LightPROF and the generalized, but resource heavy mechanism of PoG, that must be considered before utilizing either one. 

\section{Related Work}

\subsection{Prompt Engineering on LLMs}
Prompt engineering is a technique, that tries to improve the quality of LLM answers by manipulating the text input they receive in certain manners. Recently, developments in this research have lead to impressive enhancements regarding LLMs reasoning capabilities and have been improved upon numerous times. One example for this would be Chain-of-thought (CoT) \cite{b3}, a strategy that divides the reasoning process into multiple consecutive LLM thoughts, approaching the problem solution in smaller quantities, that can be handled by the LLM more consistently and leads to drastic improvements in complex reasoning tasks. A more recent derivative of CoT, that enhances the reasoning capabilities of LLMs even more is Graph of Thoughts (GoT) \cite{b4}. Whereas CoT only enables linear reasoning with intermediate steps, GoT prompts the LLM to arrange its generated thoughts in a graph structure, enabling the aggregation of multiple previous thoughts into a new one or refining thoughts using feedback loops. However, despite the improvements possible this way, these prompting strategies only utilize the LLMs  inherent knowledge, enabling further enhancements in knowledge intensive tasks only so far.  

\subsection{KG-based LLM Reasoning}
Due to the vast factual and structured knowledge KGs entail, they are a valuable asset in the enhancement of LLM reasoning. As LLMs themselves are often hindered by their shortcomings in retrieving reliable knowledge, KGs can be utilized by injecting their knowledge into the LLM, increasing inference and interpretability. While there are fundamentally different approaches to the integration of KGs into LLM reasoning, like incorporating the KG into the LLM during the pre-training process, or generating LLM prompts, that are induced with knowledge retrieved from the KG, the former strategy poses the risk of fatal memory loss during training, which is why the latter is generally the advised option. Previous approaches like Think-on-Graph (ToG) \cite{b7} retrieve reasoning paths, by exploring topic entities individually without regarding their relation to one another, underestimating the complex relationships within the graph structure. Other common problems are the high operational cost when handling KGs, due to the vast amounts of contained information, and the inability of LLMs to perceive the entirety of the information entailed by the KG, due to prompts in natural language containing redundancy. Recent models have been trying to tackle these challenges in various manners, with two of them being presented in the following chapters \cite{b5, b8}.
...
4. Reasoning Path: For a given KG, a Reasoning Path consists of a sequence of entities, that are connect with one another via relations. This is represented as a ordered list of triples, with the tail entity of one triple being identical with the head entity of the subsequent one. Reasoning paths can serve as possible solutions for KG based questions. In this case, they start of from an anchor entity, connecting it with the possible solution. For the aforementioned question, a reasoning path to the correct answer may look like this: 
$(The\_ Starry\_Night,$ $ drawn\_ by,$ $ Van\_ Gogh)$, 
$(Van\_ Gogh,$ $ born\_ in,$ $ Netherlands)$
...
Another recent framework that will be examined here is Paths-over-Graph (PoG), which introduces a methodology designed to enhance LLM reasoning by, similar to LightPROF,focusing on retrieving and processing interconnected reasoning paths that logically link topic entities. In order to retrieve the relevant knowledge however, PoG employs extensive selection and pruning strategies. The architecture of PoG is subsequently composed of four distinct, cyclic phases: Initialization, Exploration, Path Pruning, and Question Answering, which are displayed in more detail in Fig. 2.

\begin{figure*}[t]
    \centering
    \includegraphics[width=\textwidth]{pog_architecture.png}
    \caption{schematic depiction of the PoG framework \cite{b8}}
    \label{fig:breit}
\end{figure*}

\subsection{Initialization}
The process commences with the Initialization phase, which establishes the necessary foundation for efficient graph exploration through two key sub-processes: Question Subgraph Detection and Question Analysis. Initially, the framework identifies topic entities within the user's input question and identifies the corresponding entities in the KG by employing similarity matching using BERT. Subsequently, a question subgraph is constructed by expanding these topic entities to include their neighbors up to a preset maximum depth $D_max$. The resulting subgraph $G_q$ encapsulates all potentially relevant entities and relations. To manage the computational costs and potential noise inherent to large KGs, $G_q$ is thus pruned. Techniques such as node and relation clustering are employed to compress the graph by aggregating similar elements into supernodes. Furthermore, a graph reduction using bidirectional BFS is applied to identify and retain only those paths that successfully connect the topic entities, discarding irrelevant branches and unconnected nodes.

Simultaneously, the Question Analysis phase leverages the capabilities of the LLM to decompose the input question into easier, more manageable sub-questions. This decomposition serves to mitigate hallucination by focusing the model's attention on smaller problem chunks at a time. Crucially, the LLM constructs an indicator from the entirety of the topic entities, that outlines the sequence of entities and predicts the likely depth of the answer, $D_predict$, within the graph structure, that limits the subsequent exploration depth to prevent unnecessary computation expenses.

\subsection{Exploration}
Following initialization, in the Exploration phase the pruned subgraph $G_q$ is actively traversed to discover faithful reasoning paths, which, due to them being basically chains of thought, simultaneously grant a question answer, as well es an explanation. This serves better interpretability, because upon retrieving an answer, the reasoning behind this answer is automatically apparent as well. The whole exploration process is executed in three stages, that are carried out subsequently, with pruning and question answering in between, and that terminates, once a reasonable path has been found.

1. Topic Entity Path Exploration: Guided by the LLM's thinking indicator, the system prioritizes searching for paths that connect all identified topic entities in the sequence suggested by the model. The search initiates at the predicted depth and incrementally increases towards the maximum depth $D_max$, if necessary. This strategy focuses computational resources on the most logically probable connections first.
    
2. LLM Supplement Path Exploration: This stage involves exploiting the LLM's implicit knowledge, by prompting it to construct predictions based on its understanding of the reasoning paths, accounting for plausible entities, that might have been excluded from the subgraph $G_q$. These predicted entities are then verified against the KG; if valid, they are incorporated into the exploration pool to uncover supplementary paths that rigid graph traversal might miss.
    
3. Node Expand Exploration: Should the initial stages fail to provide sufficient evidence, the system expands the search space by exploring the neighbors of currently unvisited entities. Relation triples that are discovered this way are incorporated into existing reasoning paths, continued with repeated pruning and question answering. 

\subsection{Path Pruning} 
Given the sheer scale of KGs, the exploration phase can generate a substantial number of candidate paths. To optimally filter these candidates to reduce costs whilst still ensuring relevance, PoG employs a three-step beam search strategy during the Path Pruning phase. The process begins with fuzzy selection, a rapid filtering step that uses a pre-trained language model like SentenceBERT to compute the semantic similarity between the reasoning paths and the LLM's thinking indicator, with only the paths with the highest scores remaining for further analysis. These survivors then undergo precise path selection, where the LLM itself evaluates the paths, scoring and selecting the top few reasoning paths, that are predicted to lead to the correct answer. Finally, Branch Reduced Selection is integrated to optimize efficiency by pruning paths beyond a certain branching factor, keeping them concise. This is iterated, increasing the depth with every iteration, while significantly decreasing the token load on the LLM and ensuring relevance through limiting the amount of branches.

\subsection{Question Answering}
The final phase involves synthesizing the answer from the refined set of reasoning paths. To prevent the LLM from being overwhelmed by vast textual input, a path summarizing step first prompts the model to examine the retrieved paths, identifying concise, relevant triples and generating a final narrowed down reasoning path. Subsequently, the LLM evaluates whether these summarized paths provide adequate evidence to answer both the split sub-questions and the original question. If the evidence is deemed sufficient, the model generates a final response, explicitly citing the specific path used as the reasoning basis. This ensures the output is not only accurate but also interpretable, allowing the user to trace the logic back to specific KG entries. If the evidence is insufficient, the system loops back to the exploration phase to search for deeper or alternative paths, continuing the cycle until a satisfactory answer is found or resources are exhausted\cite{b5}.
...
PoG follows an entirely different approach, effectively keeping a plain-text format at all times and applying smart prompt-engineering, to convert the retrieved knowledge into a prompt format, that preserves the structural and semantic information of the KG as much as possible, without transforming it into the embedding space of the LLM. To get there, PoG makes use of the LLM through repeated effective pruning and path-selection. This leads to no additional training being required and even yields the possibility of utilizing closed-source models. The trade-off, however, is higher computational cost during the exploration and the need for a much larger context window, meaning that PoG will only excel when used with large-scale LLMs \cite{b5}.

\section{Conclusion}
In conclusion, the integration of Knowledge Graphs into Large Language Models remains a critical avenue for overcoming the inherent limitations of LLMs regarding hallucinations and lack of transparency. The examination of LightPROF and Paths-over-Graph (PoG) highlights two distinct yet complementary philosophies in addressing the challenges of multi-hop reasoning and structural preservation. LightPROF offers a parameter-efficient solution by translating structural knowledge directly into the embedding space, making it a highly effective framework for small-scale, open-source models where resource constraints are a priority \cite{b8}. Conversely, PoG leverages the advanced semantic planning capabilities of large-scale LLMs to navigate and prune graph paths in natural language, offering a robust solution compatible with proprietary, closed-source architectures \cite{b5}.
